...
with st.sidebar.form("search_form"):
    company_input = st.text_input("会社名 (株式会社抜きでOK)", value="")
    person_input = st.text_input("担当者名", value="")
    search_btn = st.form_submit_button("候補を検索")
...
if st.session_state.searched:
    ...
    if company_input:
        ...
        else: st.sidebar.warning("会社が見つかりません")
    if person_input:
        ...
        else: st.sidebar.warning("担当者が見つかりません")
...
if st.session_state.show_report:
    ...
    st.markdown(f"## 🏢 [{display_company}]({link_url if auto_url != 'ー' else '#'}) 商談調査レポート")
    ...
    with c1:
        if hp_data["address_list"]:
            ...
            st.markdown(f'<div style="display:flex;line-height:1.5;"><b>📍 住所:&nbsp;</b><div>{first}{others}</div></div>', unsafe_allow_html=True)
    ...
    if con:
        ...
        with im1:
            if card_img: st.image(card_img, caption="名刺", use_column_width=True)
            else: st.info("名刺未登録")
        with im2:
            if photo_img: st.image(photo_img, caption="写真", use_column_width=True)
            else: st.info("写真未登録")
        st.write(f"### [{con.get('Full_Name', '不明')} 様](https://crm.zoho.jp/crm/tab/Contacts/{con.get('id')})")
        st.markdown(f"<b>部署:</b> {con.get('Department', 'ー')}<br><b>役職:</b> {con.get('Title', 'ー')}<br><b>電話:</b> {con.get('Mobile') or con.get('Phone') or 'ー'}<br><b>メール:</b> {con.get('Email', 'ー')}<br><b>メモ:</b> {con.get('Description', 'ー')}", unsafe_allow_html=True)
    st.divider()
    ...
    if con:
        ...
        if not notes: st.info("ー")
            ...
            for n in notes[:3]: st.warning(f"📅 **({n.get('Modified_Time', '')[:10]})**\n\n{n.get('Note_Content')}")
    st.divider()
    c5, c6 = st.columns(2)
    with c5:
        st.write("**【商談】**")
